...
import pytest
import time
from script import sessions, DebugSession
...
def test_project_lifecycle(logged_in_client, temp_db):
    ...
    # (Note: conftest.py also patches DATA_USERS_DIR)
    import script
    project_dir = os.path.join(script.DATA_USERS_DIR, str(user_id), str(project_id))
